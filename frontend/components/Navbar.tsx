...
export default function Navbar() {
  ...
  // Dil dəyişdirmə funksiyası (URL-i dəyişir)
  const changeLanguage = (newLocale: string) => {
    const currentPath = pathname;
    // Köhnə dil prefiksini təmizləyirik (məsələn /en/teachers -> /teachers)
    const pathWithoutLocale = currentPath.replace(/^\/[a-z]{2}(?=\/|$)/, '') || '/';
    // Yeni dil ilə yönləndir
  ...
  // Hazırki URL-dən dili müəyyən et
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="relative" ref={langDropdownRef}>
              ...
              >
                {langOptions.find(l => l.code === currentLocale)?.label.split(' ')[0] || "🇦🇿"}
              </button>

              {isLangDropdownOpen && (
                <div className="absolute right-0 mt-2 w-32 bg-white rounded-xl shadow-lg border border-slate-100 overflow-hidden flex flex-col z-50 py-1 dark:bg-slate-800 dark:border-slate-700">
                  {langOptions.map((lang) => (
                    <button 
                      key={lang.code}
                      onClick={() => changeLanguage(lang.code)} 
                      className={`px-4 py-2 text-sm font-medium hover:bg-slate-50 flex items-center gap-2 dark:text-slate-300 dark:hover:bg-slate-700 ${currentLocale === lang.code ? 'text-blue-600 dark:text-blue-400' : 'text-slate-700'}`}
                    >
                      {lang.label}
                    </button>
                  ))}
