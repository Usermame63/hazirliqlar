...
export default function Navbar() {
  ...
  // YENİ DİL DƏYİŞDİRMƏ (Ən sağlam üsul)
  const changeLanguage = (newLocale: string) => {
    // Hazırki yoldan əvvəlki dil hissəsini təmizləyirik (məsələn /en/teachers -> /teachers)
    const pathWithoutLocale = pathname.replace(/^\/[a-z]{2}(?=\/|$)/, '');
    // Yeni dil ilə birlikdə yola yönləndiririk
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="relative" ref={langDropdownRef}>
              <button 
                ...
                className="w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-lg transition-colors dark:bg-slate-800 dark:hover:bg-slate-700"
              >
                {langFlags[currentLocale as keyof typeof langFlags] || "🌐"}
              </button>

              {isLangDropdownOpen && (
                <div className="absolute right-0 mt-2 w-28 bg-white rounded-xl shadow-lg border border-slate-100 overflow-hidden flex flex-col z-50 py-1 dark:bg-slate-800 dark:border-slate-700">
                  <button onClick={() => changeLanguage('az')} className="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 flex items-center gap-2 dark:text-slate-300 dark:hover:bg-slate-700">🇦🇿 Az</button>
                  <button onClick={() => changeLanguage('en')} className="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 flex items-center gap-2 dark:text-slate-300 dark:hover:bg-slate-700">🇬🇧 En</button>
                  <button onClick={() => changeLanguage('tr')} className="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 flex items-center gap-2 dark:text-slate-300 dark:hover:bg-slate-700">🇹🇷 Tr</button>
                  <button onClick={() => changeLanguage('ru')} className="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 flex items-center gap-2 dark:text-slate-300 dark:hover:bg-slate-700">🇷🇺 Ru</button>
                </div>
              )}
            </div>
            ...
            {isLoggedIn ? (
              ...
              <>
                <Link href="/login" className="text-slate-600 hover:text-blue-600 font-semibold px-4 py-2 transition-colors dark:text-slate-400 dark:hover:text-blue-500">{t('login')}</Link>
                <Link href="/register" className="bg-blue-600 text-white px-6 py-2.5 rounded-full font-semibold hover:bg-blue-700 transition-all shadow-md hover:shadow-lg dark:bg-blue-500 dark:hover:bg-blue-600">{t('register')}</Link>
              </>
            )}
          </div>
