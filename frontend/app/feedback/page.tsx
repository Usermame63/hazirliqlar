...
export default function FeedbackPage() {
  ...
  // RƏYLƏRİ BACKEND-DƏN ÇƏKMƏK
  const fetchFeedbacks = async () => {
    try {
      const res = await axios.get("https://hazirliqlar-backend.onrender.com/api/feedbacks");
      setFeedbacks(res.data);
    } catch (error) {
      console.error("Rəylər çəkilə bilmədi:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const fName = localStorage.getItem("firstName") || "İstifadəçi";
  ...
  // YENİ RƏY GÖNDƏRMƏK (Canlı Bazaya)
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(
        "https://hazirliqlar-backend.onrender.com/api/feedbacks",
        { rating, text: comment },
      ...
      fetchFeedbacks(); // Siyahını yenilə ki, dərhal görünsün
    } catch (error) {
      console.error("Rəy göndərilmədi:", error);
      alert("Xəta baş verdi. Zəhmət olmasa yenidən yoxlayın.");
    }
  };

  // RƏYƏ CAVAB YAZMAQ (Canlı Bazaya)
  const handleReplySubmit = async (feedbackId: string) => {
    ...
    try {
      await axios.post(
        `https://hazirliqlar-backend.onrender.com/api/feedbacks/${feedbackId}/reply`,
        { text: replyText },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      setReplyText("");
      fetchFeedbacks(); // Siyahını yenilə
    } catch (error) {
      console.error("Cavab göndərilmədi:", error);
      alert("Cavab yazılarkən xəta baş verdi.");
    }
  };
        ...
        <div className="space-y-4">
          ...
          {loading ? (
            ...
            <div className="bg-white p-6 rounded-2xl text-center border border-slate-100">
              <p className="text-sm text-slate-500">Hələ ki rəy yazılmayıb. İlk rəyi siz yazın!</p>
            </div>
          ) : (
            feedbacks.map((fb) => (
              <div key={fb._id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                ...
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-slate-100 text-slate-700 font-bold rounded-full flex items-center justify-center text-sm border border-slate-200 flex-shrink-0">
                      {fb.authorName ? fb.authorName[0].toUpperCase() : "İ"}
                    </div>
                    <div>
                      <div className="font-bold text-sm text-slate-900 flex items-center gap-1.5">
                        {fb.authorName || "İstifadəçi"} 
                      ...
                      <div className="text-[10px] text-slate-400 font-medium mt-0.5">
                        {new Date(fb.createdAt).toLocaleDateString("az-AZ")}
                      </div>
                    </div>
                  </div>
                ...
                {isLoggedIn && (
                  <button onClick={() => setReplyingToId(replyingToId === fb._id ? null : fb._id)} className="text-xs font-bold text-slate-500 active:text-blue-600 transition flex items-center gap-1">
                ...
                {replyingToId === fb._id && (
                  <div className="mt-3 flex gap-2">
                    ...
                    <button onClick={() => handleReplySubmit(fb._id)} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold text-xs active:bg-blue-700 transition">Göndər</button>
                  </div>
                )}
                ...
                {fb.replies && fb.replies.length > 0 && (
                  <div className="mt-3 space-y-2 pl-3 border-l-2 border-slate-200">
                    {fb.replies.map((reply: any) => (
                      <div key={reply._id || Math.random()} className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="font-bold text-xs text-slate-800">{reply.authorName || "İstifadəçi"}</span>
                          ...
                          <span className="text-[10px] text-slate-400 font-medium ml-auto">
                            {new Date(reply.createdAt || Date.now()).toLocaleDateString("az-AZ")}
                          </span>
                        </div>
