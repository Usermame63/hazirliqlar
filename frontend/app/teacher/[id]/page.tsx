...
export default function TeacherDetail() {
  ...
  useEffect(() => {
    ...
    const loadMap = async () => {
      ...
      let lat = 40.4093,
        lon = 49.8671; // Default Bakı koordinatları
      if (coordsText) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
            ...
            <p className="font-bold text-sm">
              📞 {teacher.user.phone || "Qeyd edilməyib"}
            </p>
          ...
          <div className="md:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
            ...
            <Link
              href={`/chat?userId=${teacher.user.id}`}
              className="w-full bg-blue-600 text-white font-bold py-3.5 rounded-2xl hover:bg-blue-700 transition shadow-lg shadow-blue-200 flex items-center justify-center gap-2 mt-4"
            >
              Müəllimə Mesaj Yaz
            </Link>
          </div>
