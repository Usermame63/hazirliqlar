...
export default function Home() {
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    // Səhifə ancaq ən yuxarıda (başlanğıcda) olanda dartmağa icazə ver
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (startY > 0 && typeof window !== "undefined" && window.scrollY === 0) {
      ...
      // Əgər istifadəçi barmağını aşağı çəkirsə
      if (distance > 0) {
        setPullDistance(Math.min(distance, 85)); // Maksimum 85px dartılsın
      }
    }
  };

  const handleTouchEnd = () => {
    // Əgər kifayət qədər (65px-dən çox) aşağı çəkilibsə, yenilə
    if (pullDistance >= 65) {
      setIsRefreshing(true);
      // Animasiyanın görünməsi üçün yarım saniyəlik fasilə və tam yenilənmə
      setTimeout(() => {
        window.location.reload();
      }, 400);
    } else {
      // Kifayət qədər çəkilməyibsə, ekranı əvvəlki vəziyyətinə qaytar
      setPullDistance(0);
      setStartY(0);
    }
  };
  ...
  return (
    <div 
      className="min-h-screen bg-slate-100 font-sans relative overflow-hidden"
      onTouchStart={handleTouchStart}
      onTouchMove={handleTouchMove}
      onTouchEnd={handleTouchEnd}
    >
      
      {/* YUXARIDAN AÇILAN SPİNNER (PULL TO REFRESH) */}
      <div 
        className="absolute top-0 left-0 w-full flex justify-center items-end pb-5 transition-all duration-200 z-0"
        style={{ height: `${pullDistance}px`, opacity: pullDistance / 80 }}
      >
        {isRefreshing || pullDistance >= 65 ? (
          <div className="animate-spin rounded-full h-7 w-7 border-2 border-slate-300 border-t-blue-600"></div>
        ) : (
      ...
      {/* ƏSAS SƏHİFƏ (Barmaqla aşağı sürüşən hissə) */}
      <div 
        className="bg-slate-50 min-h-screen pb-20 relative z-10 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]"
        style={{ 
        ...
        <header className="bg-white px-5 pt-12 pb-5 rounded-b-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
          <div className="flex justify-between items-center mb-5">
            <div>
              ...
              <h1 className="text-xl font-black text-slate-900 truncate max-w-[200px]">{userName}</h1>
            </div>
            {/* Profil / Qeydiyyat ikonu */}
            <Link href={isLoggedIn ? "/profile" : "/register"} className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center text-blue-600 border border-blue-100 active:bg-blue-100 transition-colors flex-shrink-0">
              {isLoggedIn ? (
                <span className="font-bold text-lg">{userName !== "Qonaq" ? userName.charAt(0).toUpperCase() : "👤"}</span>
              ) : (
